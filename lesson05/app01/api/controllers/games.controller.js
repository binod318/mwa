...
_runGeoQuery = function(req, res, offset, count){
    ...
    const point = {type: "Point", coordinates: [lng, lat]} //[2.2438757, 48.8424847]

    let minDistance = process.env.GEO_SEARCH_DEFAULT_MIN_DISTANCE;
    if(req.query && req.query.minDist){
        minDistance = req.query.minDist;
    }

    let maxDistance = process.env.GEO_SEARCH_DEFAULT_MIN_DISTANCE;
    if(req.query && req.query.maxDist){
        maxDistance = req.query.maxDist;
    }

    const query = {"publisher.location.coordinates": {
        $near: {
            $geometry: point,
            $maxDistance: parseFloat(maxDistance, process.env.NUMBER_BASE),
            $minDistance: parseFloat(minDistance, process.env.NUMBER_BASE)
        }
    }};
